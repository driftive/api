...
type FindDriftAnalysisRunByRepoAndIdempotencyKeyParams struct {
	RepositoryID   int64
	IdempotencyKey *string
}

func (q *Queries) FindDriftAnalysisRunByRepoAndIdempotencyKey(ctx context.Context, arg FindDriftAnalysisRunByRepoAndIdempotencyKeyParams) (DriftAnalysisRun, error) {
	row := q.db.QueryRow(ctx, findDriftAnalysisRunByRepoAndIdempotencyKey, arg.RepositoryID, arg.IdempotencyKey)
	var i DriftAnalysisRun
	err := row.Scan(
		&i.Uuid,
		&i.RepositoryID,
